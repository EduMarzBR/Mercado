...
list_of_files = glob.glob(
    r"C:\Users\armen\OneDrive\Estratégias\Listas\IBRA\*"
)  # * means all if need specific format then *.csv
latest_file = max(list_of_files, key=os.path.getctime)


# Caminho para a pasta que contém os arquivos CSV
caminho_pasta = r"C:\Users\armen\OneDrive\Estratégias\Base\Semanal"
...
for ativo in lista:
    ...
    caminho_arquivo = os.path.join(caminho_pasta, f"{ativo}_B_0_Semanal.csv")
    ...
    if os.path.exists(caminho_arquivo):
        ...
        if len(df) > 21:
            ...
            if diferenca_percentual > 100:
                if var > 0:
                    print(
                        f"{ativo}:{diferenca_percentual:.2f}%: fechou em alta de: {var:.2f}%"
                    )
                elif var < 0:
                    print(
                        f"{ativo}:{diferenca_percentual:.2f}%: fechou em baixa de: {var:.2f}%"
                    )
                else:
