...
for x in lista:
    ...
    max= df['Fechamento'].max()
    tdy=df['Fechamento'].iloc[-1]
    dist_t = round(((tdy-max)/max)*100,2)

    ystd = df['Fechamento'].iloc[-2]
    dist_y = round(((ystd - max) / max) * 100, 2)

    dist_high_t.append(dist_t)
    asset.append(x)

    dist_high_y.append(dist_y)


    df3=pd.DataFrame(dist_high_t)
    df3.index.name="Indice"
    df2=pd.DataFrame(asset)
    df2.index.name = "Indice"
    df1=pd.DataFrame(dist_high_y)
...
print('------------------------------------')
print('    10 ativos mais distantes da'            )
print('   máxima das últimas 52 semanas        ')
print('------------------------------------')
print (dmin)
print('-------------------------------------')
print('')
...
print('-------------------------------------')
print('    10 ativos mais próximos da '              )
print('   máxima das últimas 52 semanas         ')
print('-------------------------------------')
print(dmax)
print('-------------------------------------')
