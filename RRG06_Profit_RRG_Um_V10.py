...
# Define os limites do gráfico para incluir o zero
theta_min = min(smoothed_theta.min(), 0)
theta_max = max(smoothed_theta.max(), 0)
rot_min = min(smoothed_rotation.min(), 0)
rot_max = max(smoothed_rotation.max(), 0)

# Configura os retângulos para colorir os quadrantes
shapes = [
    # Quadrante Leading (theta > 0 e rotação > 0) - Verde
    dict(
        type="rect",
        xref="x",
        yref="y",
    ...
    # Quadrante Weakening (theta > 0 e rotação < 0) - Amarelo
    dict(
        type="rect",
        xref="x",
        yref="y",
    ...
    # Quadrante Lagging (theta < 0 e rotação < 0) - Vermelho
    dict(
        type="rect",
        xref="x",
        yref="y",
    ...
    # Quadrante Improving (theta < 0 e rotação > 0) - Azul
    dict(
        type="rect",
        xref="x",
        yref="y",
...
# Adiciona o nome do quadrante somente se o movimento estiver passando pelo quadrante
if quadrante != "Indeterminado":
    if quadrante == "FORTE":
        # theta > 0 e rotação > 0
        x_annot = (0 + theta_max) / 2
        y_annot = (0 + rot_max) / 2
    elif quadrante == "ENFRAQUECENDO":
        # theta > 0 e rotação < 0
        x_annot = (0 + theta_max) / 2
        y_annot = (rot_min + 0) / 2
    elif quadrante == "FRACO":
        # theta < 0 e rotação < 0
        x_annot = (theta_min + 0) / 2
        y_annot = (rot_min + 0) / 2
    elif quadrante == "FORTALECENDO":
        # theta < 0 e rotação > 0
        x_annot = (theta_min + 0) / 2
        y_annot = (0 + rot_max) / 2
    else:
        x_annot, y_annot = None, None

    if x_annot is not None and y_annot is not None:
        fig.add_annotation(
            x=x_annot,
            y=y_annot,
            text=quadrante,
