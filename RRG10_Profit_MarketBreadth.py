...
# -------------------------------
# Agregação: contar quantas ações estão em cada quadrante por dia
# -------------------------------
...
df_pivot = df_pivot.reindex(columns=colunas_ordem, fill_value=0)
...
for quadrante in colunas_ordem:
    if quadrante in df_pivot.columns:
        row, col = posicoes[quadrante]
        fig.add_trace(
            go.Scatter(
                x=df_pivot.index,
                y=df_pivot[quadrante],
                mode="lines",
                name=quadrante,
        ...
        last_value = df_pivot[quadrante].iloc[-1]
        # Anotação destacada com fonte maior, fundo e borda
        fig.add_annotation(
            x=last_date,
            y=last_value,
            text=f"<b>{int(last_value)}</b>",
            showarrow=True,
            arrowhead=2,
            ax=0,
            ay=-30,
