...
for ativo in acoes:
    df = mt5.copy_rates_from(
        ativo, mt5.TIMEFRAME_D1, datetime.today(), periodo
    )
    if df is not None and len(df) > 0:  # Verificação se df contém dados
        ...
        df_atual = df[df["time"].dt.year == datetime.today().year]
        maxima_ano = df_atual["high"].max()
        data_maxima_ano = df_atual[df_atual["high"] == maxima_ano]["time"].iloc[0]
        fechamento_atual = df_atual["close"].iloc[-1]
...
#        print(f"{ativo} - {data_maxima_ano} - {variacao_da_maxima_ano} - {dias}")
        ...
        minima_ano = df_atual["low"].min()
        data_minima_ano = df_atual[df_atual["low"] == minima_ano]["time"].iloc[0]
        fechamento_atual = df_atual["close"].iloc[-1]
...
var_max_ano = pd.DataFrame(var_max_ano, columns=["Ativo", "Data da Máxima no Ano", "Variação", "Dias da Máxima no Ano"])
var_max_ano = var_max_ano.sort_values(by="Variação", ascending=False)

var_min_ano = pd.DataFrame(var_min_ano, columns=["Ativo", "Data da Mínima no Ano", "Variação", "Dias da Mínima no Ano"])
var_min_ano = var_min_ano.sort_values(by="Variação", ascending=False)


print("Ações que fizeram a Máxima no Ano:")
max_ano_dia_atual = var_max_ano[var_max_ano["Dias da Máxima no Ano"] == 0]
print(max_ano_dia_atual)
print("")
print("Ações que fizeram a Mínima no Ano:")
min_ano_dia_atual = var_min_ano[var_min_ano["Dias da Mínima no Ano"] == 0]
print(min_ano_dia_atual)
print("")
print("")
print(f"{len(max_ano_dia_atual)} ações fizeram a máxima no Ano")
print(max_ano_dia_atual["Ativo"].tolist())
print(f"{len(min_ano_dia_atual)} ações fizeram a mínima no Ano")
