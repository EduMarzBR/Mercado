...
def identificar_FFFD(df):
    ...
    for i in range(20, len(close)):
        if close.iloc[i - 2] > bs.iloc[i - 2]:
            if close.iloc[i - 1] < bs.iloc[i - 1] and close.iloc[i - 1] > m.iloc[i - 1]:
                signals.iloc[i - 1] = 1
...
def preco_entrada(df):
    ...
    for i in range(20, len(df)):
        if df["FFFD"].iloc[i - 1] == 1:
            ...
            p_entrada.iloc[i] = df["low"].iloc[i - 2] - 0.01
...
def preco_stop_loss(df):
    ...
    for i in range(20, len(df)):
        if df["FFFD"].iloc[i - 1] == 1:
            ...
            p_stop_loss.iloc[i] = df["high"].iloc[i - 2] + 0.01
    return p_stop_loss
        ...
        if (
            ...
            if (
                ...
                if (
                    ...
                    if (
                        (df["Preço Entrada"].iloc[i] - df["Preço Objetivo"].iloc[i])
                        / df["Preço Entrada"].iloc[i]
                    ) > abs(
                        (df["Preço Entrada"].iloc[i] - df["Preço Stop"].iloc[i])
                        / df["Preço Entrada"].iloc[i]
                    ):
                        entrada.iloc[i] = df["low"].iloc[i - 1] - 0.02
                        objetivo.iloc[i] = m.iloc[i]
                        stop_loss.iloc[i] = df["high"].iloc[i - 1] + 0.02
        elif (
            df["FFFD"].iloc[i - 2] == 1
            and df["low"].iloc[i - 1] >= df["low"].iloc[i - 2] - 0.01
            and df["high"].iloc[i - 1] <= df["high"].iloc[i - 2] + 0.01
            and df["low"].iloc[i] <= df["low"].iloc[i - 2] - 0.01
            #and real_volume.iloc[i] > real_volume_mean.iloc[i]
        ):
            if (
                df["high"].iloc[i] >= df["low"].iloc[i - 2] - 0.01
                and df["open"].iloc[i] < df["high"].iloc[i - 2]
            ):
                # Filtra operações que tenham potencial de ganho maior do que potencial de perda
                if (
                    df["Preço Objetivo"].iloc[i] is not None
                    ...
                    if (
                        ...
                        entrada.iloc[i] = df["low"].iloc[i - 2] - 0.02
                        objetivo.iloc[i] = m.iloc[i]
                        stop_loss.iloc[i] = df["high"].iloc[i - 2] + 0.02
    operacoes = pd.concat(
        [
            entrada,
            objetivo,
...
def screening_FFFD(df):
    ...
    for i in range(20, len(close)):
        if close.iloc[-2] > bs.iloc[-2]:
            if close.iloc[-1] < bs.iloc[-1] and close.iloc[-1] > m.iloc[-1]:
                if real_volume.iloc[-1] > real_volume_mean.iloc[-1]:
                    signals = 1
        elif close.iloc[-3] > bs.iloc[-3]:
            if close.iloc[-2] < bs.iloc[-2] and close.iloc[-2] > m.iloc[-2]:
...
def screening_preco_stop_loss(signals, df):
    ...
    if signals == 1:
        ...
        p_stop_loss = df["high"].iloc[-2] + 0.01
...
if len(analise) > 0:
    ...
    merged_analise.to_excel("analise_FFFD_venda.xlsx", index=False)
    print("Estudo salvo em analise_FFFD_venda.xlsx")
