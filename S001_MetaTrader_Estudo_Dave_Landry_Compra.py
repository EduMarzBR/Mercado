    ...
    for i in range(3, len(df)):
        if (
            ...
            if (
                ...
                and df["low"].iloc[i - 1] < df["low"].iloc[i - 3]
            ):
                signals.iloc[i - 1] = 1
...
def preco_stop_loss(df):
    ...
    for i in range(3, len(df)):
        if (
            ...
            if df["low"].iloc[i] <= df["high"].iloc[i - 1] - 0.01:
...
def screening_preco_objetivo(signals, df):
    p_objetivo = 0
    if signals == 1:
        p_objetivo = (2 * (df["high"].iloc[-1] - df["low"].iloc[-1])) + df["high"].iloc[
            -1
        ]
...
merged_analise.to_excel("analise_Dave_Landry_compra.xlsx", index=False)
# se lista de screening não estiver vazia, prossegue com o processo
if screening:
...
print(f"")
print(
    f"Estratégia Dave Landry de Compra com histórico de acerto maior ou igual a 50%. - {dt} "
