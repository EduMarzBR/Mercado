...
fig.add_trace(
    go.Scatter(x=df3_250.index, y=df3_250["addc"], mode="lines", name="LAD", line=dict(color="blue")),
    row=1,
    col=1,
)
fig.add_trace(
    go.Scatter(x=df3_250.index, y=df3_250["addc_MMA"], mode="lines", name="MMA21", line=dict(color="red")),
    row=1,
    col=1,
)

# # Adicione o gráfico de linha do Índice
fig.add_trace(
    go.Scatter(
        x=ind_250.index,
...
fig.update_layout(
    title_text=f"LInha de Avanço e Declínio - {indice} - {dt}",
    showlegend=True,
...
fig.update_yaxes(title_text="LAD vs. MMA21", row=1, col=1, title_font=dict(size=20))
