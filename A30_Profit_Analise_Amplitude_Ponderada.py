...
resultados["NH_NL_I"] = (
    (resultados["Total_Nova_Alta"] - resultados["Total_Nova_Baixa"])
    / (resultados["Total_Nova_Alta"] + resultados["Total_Nova_Baixa"])
) * 100
resultados.fillna({"NH_NL_I": 0}, inplace=True)
resultados["NH_NL_I_Media_Movel_10"] = resultados["NH_NL_I"].rolling(window=20).mean()


# Calcular a média móvel dos últimos 20 dias de resultados['NH_NL_I']
resultados["NH_NL_I_Media_Movel"] = (
    resultados["NH_NL_I_Media_Movel_10"].rolling(window=30).mean()
)
...
for index, row in resultados.iterrows():
    if (
        ...
        and index > 0
        and row["Cruzamento"] != resultados.iloc[index - 1]["Cruzamento"]
    ):
...
for index in range(1, len(resultados)):
    if (
        resultados.loc[index, "Mudança"] == 1
        and resultados.loc[index, "Cruzamento"] == 1
    ):
        resultados.loc[index, "Distancia Inicial"] = abs(
            resultados.loc[index, "NH_NL_I_Media_Movel_10"]
            - resultados.loc[index, "NH_NL_I_Media_Movel"]
        )
    elif (
        resultados.loc[index, "Mudança"] != 1
        and resultados.loc[index, "Cruzamento"] == 1
    ):
        resultados.loc[index, "Distancia Inicial"] = resultados.loc[
            index - 1, "Distancia Inicial"
        ]
    elif (
        resultados.loc[index, "Mudança"] == 1
        and resultados.loc[index, "Cruzamento"] == -1
...
for i in range(len(resultados)):
    resultados.loc[i, "Distancia Atual"] = abs(
        resultados.loc[i, "NH_NL_I_Media_Movel_10"]
        - resultados.loc[i, "NH_NL_I_Media_Movel"]
    )

# Faz um loop para calcular a diferença entre a distância inicial e a distância atual
for i in range(len(resultados)):
    resultados.loc[i, "Distancimento"] = (
        resultados.loc[i, "Distancia Atual"] - resultados.loc[i, "Distancia Inicial"]
    )
...
fig.update_layout(
    title_text=f"Análise de Amplitude do {indice} - {dt}",
    showlegend=True,
    xaxis_rangeslider_visible=False,
)
fig.update_yaxes(title_text="LAD vs. MMA30", row=2, col=1)
fig.update_yaxes(title_text="NH NL I vs. MMA30", row=1, col=1)
fig.update_yaxes(title_text="Somatória do IFR", row=3, col=1)
fig.update_yaxes(title_text=f"{indice}", row=4, col=1)
